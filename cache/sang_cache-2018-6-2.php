...
$fileName = __DIR__.DIRECTORY_SEPARATOR.'b'.DIRECTORY_SEPARATOR.'test1.docx';
...
class Word2Json
{
					...
					if ($paragraph->nodeType == XMLREADER::ELEMENT && $paragraph->name === 'w:p') {
						...
						while($paragraph->read()) {
							if($paragraph->nodeType == XMLReader::ELEMENT) { 
								...
								// if((strstr($t,$ts) == false) || (strstr($cache_t[count($cache_t)-1],$ts) ==false)) $t .= $ts;else continue;
	...
	private function checkImageFormating($rIdIndex) {
		// echo (strstr('word/media/image19.gif','word/media/image1'));\
		// echo mb_strlen("word/media/image1","utf-8");die;
		// $a = ($rIdIndex-8 < 10) ? mb_substr('word/media/image19.gif',mb_strlen("word/media/image1","utf-8")+1,1, 'utf-8') : '';
		// echo $a;die;
		// echo strstr("word/media/image19.gif","word/media/image1");
